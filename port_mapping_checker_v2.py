...
# Check if the script is running as a standalone exe
if getattr(sys, 'frozen', False):
    # If it's an exe, use the folder containing the exe
    script_dir = os.path.dirname(sys.executable)
else:
    # If it's a script, use the folder containing the script
...
ports_to_search = config.get("DEFAULT", "ports_to_search").strip('"').split(',')

# Define the desired service
desired_service = config.get("DEFAULT", "desired_service").strip('"')
...
desired_service_label = tk.Label(root, text=f"The Service we want on ports 6000/6001 should be '{desired_service}'")
desired_service_label.pack(pady=10)
